...
void QCPBars::draw(QCPPainter *painter)
{
  ...
  double x1 = mKeyAxis.data()->coordToPixel(1);
  double x2 = mKeyAxis.data()->coordToPixel(2);
  double scale = (x2 - x1)/1.;
  ...
  for (it = lower; it != upperEnd; ++it)
  {
    ...
    if (mShowValue)
    {
        ...
        painter->setFont(QFont(QStringLiteral("Î¢ÈíÑÅºÚ"), scale/50.));
        QRectF barRect = barPolygon.boundingRect();
        QRectF textRect(QPointF(barRect.left(),barRect.top()-scale/10.),QPointF(barRect.right(),barRect.top()));
        painter->drawText(textRect,Qt::AlignHCenter,QString::number(it.value().value));
    }
  }
}
